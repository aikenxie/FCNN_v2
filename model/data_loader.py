    ...
    def process(self):
        ...
        for idx,raw_path in enumerate(tqdm(self.raw_paths)):
            ...
            for ievt in range(np.shape(npzfile['x'])[1]):
                ...
                x = inputs[:,3:10]
                x=np.insert(x,0, inputs[:,0]*np.cos(inputs[:,2]),axis=1) #pX
                x=np.insert(x,1, inputs[:,0]*np.sin(inputs[:,2]),axis=1) #pY
                x=np.insert(x,2, inputs[:,0],axis=1) #pt
                x=np.insert(x,3, inputs[:,1],axis=1) #eta
                x=x[x[:,8]!=-999]
                x=x[x[:,9]!=-999]
                #print(x[0])
                x = np.nan_to_num(x)
...
#                                                             generator=torch.Generator().manual_seed(random_seed))
    ...
    dataloaders = {
        'train':  DataLoader(train_subset, batch_size=batch_size,num_workers=8, shuffle=False),
        'test':   DataLoader(val_subset, batch_size=batch_size,num_workers=8, shuffle=True)
        }
    return dataloaders
